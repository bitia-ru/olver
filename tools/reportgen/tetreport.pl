...
foreach (keys %{$handler->{'grouping'}}){
    if($_ =~ /known bugs;([^;]+);([^;]+);([^;]+)/){
        my $pack = $1 ne 'unknown package' ? $1 : 'other.other';
        my $spec = $2 ne 'unknown model operation' ? $2 : 'others';
        my $bid = $3;
        ...
        if ($_ =~ /target function failures;([^;]+);([^;]+)/){
            $pack = $1 ne 'unknown package' ? $1 : 'other.other';
            $spec = $2 ne 'unknown model operation' ? $2 : 'others';
        }
