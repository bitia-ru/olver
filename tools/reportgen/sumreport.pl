...
my $cur_bug = '';
my @cur_grouping;
my $cdata; # actually it's content
...
sub end_element {
	...
	$cdata =~ s/(http\:\/\/www\.nabble\.com.+\.html)/<a href="$1">$1<\/a>/g;
	$cdata =~ s/(http\:\/\/bugs\..+show_bug\.cgi\?id\=\d+)/<a href="$1">$1\<\/a>/g;
	$cdata =~ s/(http\:\/\/sources\..+show_bug\.cgi\?id\=\d+)/<a href="$1">$1\<\/a>/g;
	$cdata =~ s/(http\:\/\/linuxtesting\.\w+\/results\/report\?num\=S\w+)/<a href="$1">$1\<\/a>/g;
	$self->{'bug'}{$cur_bug} = $cdata;
    }
    elsif($name eq 'FailureGroup'){
    	pop (@cur_grouping);
    }
...
<tr><td>No data</td><td>$scen_status{'nodata'}</td></tr>
...
if($scen_status{'nodata'}){
	print OUT <<HTML;
<h1><a class='main href='#'>Scenarios without traces</a></h1>
<table class='border'>
HTML
